...
use napi::bindgen_prelude::{spawn, ReadableStream};
use napi::Env;
use napi::Result;
use napi_derive::napi;

use crate::session_context::{
  binary_cols_to_strings, record_batches_to_ipc, QueryResult, SessionContext,
};
...
impl SedonaDataFrame {
  ...
  pub async fn select(&self, cols: Vec<String>) -> Result<SedonaDataFrame> {
    ...
    let exprs: Vec<datafusion::prelude::Expr> = cols.iter().map(col).collect();
  ...
  #[napi]
  pub fn to_view(&self, ctx: &SessionContext, name: String, overwrite: Option<bool>) -> Result<()> {
