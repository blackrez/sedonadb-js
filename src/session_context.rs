...
use lru::LruCache;
use napi::bindgen_prelude::{block_on, spawn, ReadableStream};
use napi::Env;
use napi::Result;
...
pub fn ipc_to_rows(arrow_ipc: Vec<u8>) -> Result<Vec<Vec<String>>> {
  ...
  for batch_result in reader {
    let batch =
      batch_result.map_err(|e| napi::Error::from_reason(format!("Arrow IPC batch: {e}")))?;
      ...
      let strings: Vec<String> = (0..array.len())
        .map(|j| {
          if array.is_null(j) {
            ...
            arrow::util::display::array_value_to_string(array.as_ref(), j).unwrap_or_default()
          }
        })
        .collect();
...
/// Convert date/time columns to epoch milliseconds (Int64) so serde_arrow
/// produces numbers instead of ISO strings. JS consumers can `new Date(ms)`.
fn date_cols_to_millis(batch: &RecordBatch) -> Result<RecordBatch> {
  let schema = batch.schema();
  let original_fields = schema.fields();
  let mut new_fields: Vec<arrow::datatypes::Field> = Vec::with_capacity(batch.num_columns());
  let mut new_columns: Vec<ArrayRef> = Vec::with_capacity(batch.num_columns());
    ...
    match dt {
      DataType::Date32 => {
        ...
        let arr = batch
          ...
          .unwrap();
        let millis: arrow::array::Int64Array = arr
          .iter()
          .map(|opt| opt.map(|d| d as i64 * 86_400_000_i64))
          .collect();
        new_fields.push(arrow::datatypes::Field::new(
          field.name(),
          DataType::Int64,
        ...
        new_columns.push(Arc::new(millis));
      }
      DataType::Date64 => {
      ...
      DataType::Timestamp(unit, _) => {
        use arrow::array::*;
        let millis: Int64Array = match unit {
          arrow::datatypes::TimeUnit::Second => {
            let arr = batch
              ...
              .unwrap();
            arr.iter().map(|opt| opt.map(|s| s * 1000)).collect()
          }
          ...
          arrow::datatypes::TimeUnit::Microsecond => {
            let arr = batch
              ...
              .unwrap();
            arr.iter().map(|opt| opt.map(|us| us / 1000)).collect()
          }
          arrow::datatypes::TimeUnit::Nanosecond => {
            let arr = batch
              ...
              .unwrap();
            arr.iter().map(|opt| opt.map(|ns| ns / 1_000_000)).collect()
          }
        };
        new_fields.push(arrow::datatypes::Field::new(
          field.name(),
...
async fn collect_to_query_result(df: DataFrame) -> Result<QueryResult> {
  ...
  // then date/time columns are converted to epoch millis (numbers).
  let mut rows = Vec::with_capacity(num_rows as usize);
  for batch in &batches {
    let safe = binary_cols_to_strings(batch)?;
    let safe = date_cols_to_millis(&safe)?;
...
impl SessionContext {
  ...
  pub async fn sql(&self, query: String, params: Option<serde_json::Value>) -> Result<QueryResult> {
    if let Some(params) = params.as_ref() {
      ...
      if let Some(cached_plan) = plan {
        ...
        let param_values = build_param_values(params)?;
  ...
  /// - An object for named params (`$name` in SQL)
  #[napi]
  pub fn stream_sql<'env>(
    &self,
    env: Env,
    query: String,
    params: Option<serde_json::Value>,
  ) -> Result<ReadableStream<'env, Vec<String>>> {
